...
On a build where the label is not captured, every number here is zero and that is the
check: `step` in particular must read 0.00, the same as the build before it. It is kept on
`main` because it is how a build that captures the label — the parked travel work on
`issue-54-travel-parked`, and #58 — gets judged.
