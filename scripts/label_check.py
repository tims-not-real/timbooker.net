"""The label's acceptance check for #51: it does not move on a mobile tab switch, and it
still travels exactly as far as it did on desktop.
...
worst deviation from that resting frame over every leg. Above it the height animation
still runs, and what is checked is that each leg travels the same distance it travels on
another build — pass the other tree and the two are printed side by side.

Which widths count as mobile is not a list kept here. The page is asked: `--hero-cols` is
declared beside `grid-template-columns` in `site_style.py`, so the stylesheet owns the
line and this reads it back, the same way the creature reads back `--crit-cell` (#49). A
build from before #51 has no such property and reports 0; this tree's answer is what
sorts the widths, so the other tree is measured on the same legs either way.
...
COLS = """() => parseInt(getComputedStyle(document.querySelector('.hero'))
                         .getPropertyValue('--hero-cols'), 10) || 0"""
...
print('one column, so the label holds still. --hero-cols is 1 at %s'
...
print()
print('two columns, so the height animation still runs and still travels as far')
print()
