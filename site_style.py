...
/* ---- and moving between them inside one document ----
   ...
   root element by script: from the first render, except where the document is arriving
   under a cross-document transition from one of those standalone pages, when it is set
   at that transition's end (#59). The old page was captured with the root, the label and
   the grain named, and this one has to be captured the same way or the three groups have
   nothing to cross to and the grain fades out as a veil over the whole page. With no
   JavaScript the class is never set and nothing below applies.
...
/* The one group there is does not animate, and that is deliberate. The transition owns
   the cross-fade and the live row owns the travel (#60): home's label is 145px taller
   than the others, and the router animates the hero's row from the one height to the
   other on the live page, .18s, the same clock as the snapshots. A browser re-targets a
   group at its live element on every frame, so an animating group would interpolate
   towards a target that was itself moving, and the body trailed the label by up to 36px
   mid-swap when that was tried. Pinned, the group sits exactly on the live body, and
   the live body is where the row puts it, so the gap between the label's bottom edge
   and the first line of body text is 55px on every frame of every leg.

   Unpinned with nothing moving the live page (#54, the build between) it was worse the
   other way: the body's snapshot slid from where it was captured to where the new page
   wanted it while the label underneath, which is not captured, had already snapped,
   and the gap opened from 55px to 200px and closed again over the swap. A pinned group
   also keeps a swap that changes the scroll position from sliding.

   Two ways of travelling the label were built and are not this. A CSS transition on
   the row does not run from a length that has only just been set, because the
   before-change style is the last rendered one; measured, `auto -> 613px -> 468px` in
   one task interpolated nothing. The label named in parts (`issue-54-travel-parked`)
   travels well, 35 edge positions and a 15px largest step, but it captures the label,
   which takes the fixed grain on `body::after` off it for the length of every swap: a
   step of 4.29 of 255 on the blue and the mottle flattens with it, by
   scripts/grain_check.py. The live row captures nothing, so the grain never moves. */
