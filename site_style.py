...
/* ---- and moving between them inside one document ----
   ...
   column and the page body. The grain keeps the name it already had, so it is lifted
   into the transition layer and lands on top of those two rather than being left in a
   root snapshot that no longer exists. */
...
html.app .plate > .viz:not([hidden]){view-transition-name:plate}
html.app [data-page]:not([hidden]){view-transition-name:page}
