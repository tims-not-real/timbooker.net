...
.hero{display:grid; grid-template-columns:1fr 23rem; gap:2.5rem; align-items:stretch;
  /* The count is declared beside the tracks so that the router can ask how many there
     are without keeping its own copy of 880. It animates the hero's height across a swap
     and gives the label's row the whole of it, which is right beside a plate and wrong
     stacked above one; the breakpoint below is where that changes, and one copy of that
     rule is enough. Same arrangement as --crit-cell (#51, #49). */
  --hero-cols:2}
...
@media(max-width:880px){
  ...
  .hero{grid-template-columns:minmax(0,1fr); --hero-cols:1}
