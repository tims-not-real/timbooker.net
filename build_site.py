...
//
// So the plate counts them, and what it counts is checked rather than said: the count
// came off the caption on 2026-09-09 and the glider sentence it fed on 2026-09-15, and
// scripts/life_check.py reads the census over the board the reader is looking at, every
// painted frame. Connected components of the board, 8-connected and across the wrap,
// each matched against a catalogue by shape alone, up to translation and the eight
// symmetries of the square. Whatever matches nothing is counted rather than dropped:
// most of a running board is not catalogue objects at all, and a census that quietly
// binned the remainder would be reporting a tidier board than the one on the screen.
...
// skip — the ticks since the last step, and EVERY what a step costs. One in four.
var skip = 0, EVERY = 4;
...
// watch.
//
// The census used to be written out under it, a second line counting the blocks and the
// blinkers and the gliders. It came off on 2026-09-09, and the sentence it fed — the
// plate naming a glider when it saw one — came off on 2026-09-15, because the field's
// running line was asked to be static and a line that turns on whatever happens to be on
// the board is not. Nothing in here reads the board any more. One line, and it is still
// written to one: .note is 4.4em, the plate column is what sets the label's height on
// About, and a line that wrapped would move it.
function say(){
...
return { tick: function(){
  ...
  // A quarter speed, and it is a skipped tick rather than a clock: the plate keeps the
  // frame-tied character it has always had, and a quarter is exactly a quarter on any
  // display. A skipped tick does nothing at all — no step, no paint and no count. The
  // board has not changed, so there is nothing to repaint and nothing to recount.
  if (++skip < EVERY) return true;
  skip = 0;
  board.step();
  paint();
  count();
