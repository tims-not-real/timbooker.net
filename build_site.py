...
var objs = TBcensus(board.cells(), N);   // how the caption knows a glider is on the board
...
// blinkers and the gliders. It came off on 2026-09-09: it changed on every step, and a
// list of nine numbers that never holds still is not something a reader can read. What
// the counting was for is still here, one sentence of it — the plate knows a glider when
// it sees one, and says so — and that was always the line the count was building to.
// One line, and it is still written to one: .note is 4.4em, the plate column is what
// sets the label's height on About, and a line that wrapped would move it.
function say(){
...
return { tick: function(){
  ...
  // display. A skipped tick does nothing at all — no step, no paint, no count and no
  // census. The board has not changed, so there is nothing to repaint and nothing to
  // recount, and the census is still the expensive call on this plate even now that it
