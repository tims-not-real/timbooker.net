...
// armed — whether the settle detector is allowed to fire. A settled board is still
//         settled, so it is not, until the board has been seen going somewhere.
var skip = false, armed = false;
...
// the settle detector has seen nothing yet and the cap is 4800 again.
function load(name){
  scene = name;
  SCENES[name]();
  quiet = false; capped = false; armed = false; capAt = CAP; touchedAt = -1e9;
...
// A scene button lays the scenario out and paints it, and it does not start the loop. It
// never has: everything the reader can do while the plate is stopped paints its own frame
// and leaves it stopped, which is what keeps the run button's word true. Making this one
// press an exception would mean a scene button silently undoing a Pause the reader asked
// for, and there would then be a press that says Resume and a press that resumes.
...
// stopped and the button's word is true whichever of them did it. Resume asks the runtime
// for it back. Everything the reader can do while it is stopped paints its own frame.
function halt(){ paused = true; runner.textContent = 'Resume'; }

// Resume re-arms both exits, and it has to. A settled board is still settled, so a
// detector that fired on the first tick after a resume would stop the plate again and the
// button would have done nothing, twice. The board has to be seen not repeating once
// before it may stop on repeating again; ash the reader has asked to run goes on running,
// which is what they asked for. A board already at the cap gets a fresh run of them.
runner.addEventListener('click', function(){
  paused = !paused;
  runner.textContent = paused ? 'Resume' : 'Pause';
  if (!paused){
    armed = false; capped = false;
...
var ink = -1, lx = 0, ly = 0;
function touch(){
  // The board is the reader's from here, so the settle detector goes back to unarmed
  // along with it: what they have just drawn gets seen going somewhere before the plate
  // is allowed to call it finished. It is the same re-arming Resume does, for the same
  // reason. `capped` is left alone: a board past the cap is still past the cap, and the
  // caption goes on saying so once the hand-drawn line has had its 1200ms.
  quiet = false; armed = false; touchedAt = performance.now();
  look(); say(); paint();
}
...
return { tick: function(){
  ...
  look();
  quiet = board.repeats();
  // Seen going somewhere, so it may be called finished when it stops going anywhere.
  // repeats() compares against one and two generations back, so it fires one step past a
  // still life and two past a blinker, and the generation it rests on is that far past
  // where the board truly settled.
  if (!quiet) armed = true;
  capped = board.gen() >= capAt;
  var done = (quiet && armed) || capped;
